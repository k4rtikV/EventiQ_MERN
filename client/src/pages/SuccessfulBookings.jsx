        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-8 dark:bg-gray-900 dark:border-gray-800">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 dark:text-gray-400">All confirmed bookings with completed payment.</p>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden dark:bg-gray-900 dark:border-gray-800">
                <ul className="divide-y divide-gray-100">
                    {bookings.length === 0 ? (
                        <li className="p-6 text-gray-500 text-center dark:text-gray-400">No successful bookings yet.</li>
                    ) : bookings.map(booking => (
                        <li key={booking._id} className="p-6 hover:bg-gray-50 dark:hover:bg-gray-800 transition border-l-4 border-l-green-400">
                            ...
                            <div className="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-100 text-sm dark:bg-gray-800 dark:border-gray-800">
                                <p className="text-gray-700 flex items-center gap-2 mb-1 dark:text-gray-200">
                                    <span className="font-bold w-20 text-gray-500 uppercase text-xs dark:text-gray-400">User:</span>
                                    <span className="font-semibold">{booking.userId?.name}</span>
                                    <span className="text-gray-400">({booking.userId?.email})</span>
                                </p>
                                <p className="text-gray-700 flex items-center gap-2 mb-1 dark:text-gray-200">
                                    <span className="font-bold w-20 text-gray-500 uppercase text-xs dark:text-gray-400">Amount:</span>
                                    <span className="font-semibold">{booking.amount === 0 ? 'Free' : `₹${booking.amount}`}</span>
                                </p>
                                <p className="text-gray-700 flex items-center gap-2 mb-1 dark:text-gray-200">
                                    <span className="font-bold w-20 text-gray-500 uppercase text-xs dark:text-gray-400">Date:</span>
                                ...
                                {booking.eventId && (
                                    <p className="text-gray-700 flex items-center gap-2 mt-2 pt-2 border-t border-gray-200 dark:text-gray-200 dark:border-gray-700">
                                        <span className="font-bold w-20 text-gray-500 uppercase text-xs dark:text-gray-400">Seats:</span>
