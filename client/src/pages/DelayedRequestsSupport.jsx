...
const DelayedRequestsSupport = () => {
    ...
    return (
        <div className="mx-auto max-w-7xl py-6">
            <div className="mb-4 text-sm text-gray-500 dark:text-gray-400"><button onClick={() => navigate('/admin')} className="font-bold text-blue-700 hover:text-blue-900">Admin Dashboard</button> <span className="mx-2">›</span> Delayed Requests Support</div>
            <div className="mb-7 rounded-2xl bg-gray-950 p-7 text-white shadow-lg">
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    ...
                    <button onClick={() => navigate('/admin')} className="rounded-xl border border-white/70 bg-white px-5 py-3 font-black text-black shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:bg-blue-50 hover:text-blue-700 hover:shadow-lg active:translate-y-0 active:scale-[0.98] focus:outline-none focus:ring-4 focus:ring-blue-300/40 dark:bg-gray-900 dark:text-white">Back to Admin Dashboard</button>
                </div>
            </div>

            <div className="mb-6 grid gap-3 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm md:grid-cols-[1fr_auto_auto] dark:bg-gray-900 dark:border-gray-700">
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search user, event, email or booking ID" className="rounded-xl border border-gray-300 px-4 py-3 outline-none focus:ring-2 focus:ring-blue-200 dark:border-gray-600" />
                <select value={type} onChange={(e) => setType(e.target.value)} className="rounded-xl border border-gray-300 px-4 py-3 font-semibold dark:border-gray-600"><option value="all">All request types</option><option value="ticket_delay">Ticket delays</option><option value="refund_delay">Refund delays</option></select>
