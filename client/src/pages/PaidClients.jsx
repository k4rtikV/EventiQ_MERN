        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-8 dark:bg-gray-900 dark:border-gray-800">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 dark:text-gray-400">Confirmed bookings with completed payment and saved address details.</p>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden dark:bg-gray-900 dark:border-gray-800">
                <ul className="divide-y divide-gray-100">
                    {clients.length === 0 ? (
                        <li className="p-6 text-gray-500 text-center dark:text-gray-400">No paid clients found.</li>
                    ) : clients.map(clientBooking => (
                        <li key={clientBooking._id} className="p-6 hover:bg-gray-50 dark:hover:bg-gray-800 transition border-l-4 border-l-blue-400">
                            <div className="flex flex-col gap-3">
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                                    <div>
                                        <h4 className="font-bold text-gray-900 text-lg leading-tight dark:text-gray-100">{clientBooking.userId?.name || 'Unknown User'}</h4>
                                        <p className="text-sm text-gray-500 dark:text-gray-400">{clientBooking.userId?.email}</p>
                                    </div>
                                ...
                                <div className="bg-gray-50 rounded-lg p-4 border border-gray-100 text-sm dark:bg-gray-800 dark:border-gray-800">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <p className="font-semibold text-gray-700 dark:text-gray-200">Event</p>
                                            <p className="text-gray-500 dark:text-gray-400">{clientBooking.eventId?.title || 'Deleted Event'}</p>
                                        </div>
                                        <div>
                                            <p className="font-semibold text-gray-700 dark:text-gray-200">Amount</p>
                                            <p className="text-gray-500 dark:text-gray-400">{clientBooking.amount === 0 ? 'Free' : `₹${clientBooking.amount}`}</p>
                                        </div>
                                        <div>
                                            <p className="font-semibold text-gray-700 dark:text-gray-200">Phone</p>
                                            <p className="text-gray-500 dark:text-gray-400">{clientBooking.address?.phone || 'Not provided'}</p>
                                        </div>
                                        <div>
                                            <p className="font-semibold text-gray-700 dark:text-gray-200">Address</p>
                                            <p className="text-gray-500 dark:text-gray-400">{clientBooking.address ? `${clientBooking.address.street}, ${clientBooking.address.city}, ${clientBooking.address.state}, ${clientBooking.address.zip}, ${clientBooking.address.country}` : 'No address saved'}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
